					...
					<div class="card card-file" id="getTaskFiles1">
																	...
																	<div class="card-body" >
		<h6><a href="" id="folder_name1" >{{$directories[$i]}}</a>
			<a href="" id="path1">{{str_replace("public/FileManager/","",$path)}} </a>
		</h6>
																		<span>10.45kb</span>
																	</div>
							...
							<div class="card-footer">Last Modified: {{date("H:i:s" ,Storage::lastModified($files[$i]))}}</div> 
													...
													<div class="file-content-inner" id="displayTaskFolderFiles">

													</div>

	
<!-- Task Followers Modal -->
<div id="create_folder_project" class="modal custom-modal fade" role="dialog">
...
<!-- /Task Followers Modal -->
	...
	<script>
				...
				$(document).on('click', '#getTaskFiles1',  function(e) {
          ...
          var folder_name = $(this).find("#folder_name1").text();
           var path = $(this).find("#path1").text();
          ...
          $.ajax({
              ...
              success: function(response)
              {
                  if(response.status == "SUCCESS")
                  {
                      ...
                      $('#task-folders').attr("style","display:none")
                      $('#displayTaskFolderFiles').html();
                      $("#displayTaskFolderFiles").append(response.data.gettaskfileshtml);
